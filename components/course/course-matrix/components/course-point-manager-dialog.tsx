...
import { matchesCoursePointKeyword, sortCoursePointsByTitle } from "../utils/course-matrix-utils"
...
export const CoursePointManagerDialog = () => {
  ...
  const handleSingleDelete = async (coursePointId: number) => {
    ...
    try {
      const response = await api.coursePoints.deleteCoursePoint(coursePointId)
      if (response.error) {
